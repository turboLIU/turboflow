...
class TurboBASE(object):
    ...
    def activation(self, *args, **kwargs):
        ...
        if act_type == "relu":
            ...
            return  self.mish(args[0])
        else:
            return NotImplementedError

    def mish(self, x):
        y = tf.compat.v1.log(1 + tf.exp(x))
        return x * tf.tanh(y)
    ...
    def instance_norm(self, *args, **kwargs):  # ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if self.load_premodel:
            ...
            with tf.device("/cpu:0"):
                gama = tf.compat.v1.get_variable(shape=shape, initializer=tf.ones_initializer, trainable=True, name="%s_gama" % name)
                beta = tf.compat.v1.get_variable(shape=shape, initializer=tf.zeros_initializer, trainable=True, name="%s_beta" % name)
                pop_mean = tf.compat.v1.get_variable(shape=shape, initializer=tf.zeros_initializer, trainable=True,
                                                     name="%s_pop_mean" % name)
                pop_variance = tf.compat.v1.get_variable(shape=shape, initializer=tf.ones_initializer, trainable=True,
                                                         name="%s_pop_variance" % name)
        if kwargs["training"] == True:
            average_mean, average_varance = tf.compat.v1.nn.moments(args[0], axes=[1, 2], keep_dims=True, name=name)
    ...
    def ConvBlock_FuseBN_with_jsonParams(self, x, in_channels, out_channels, kernel_size, stride=1,
        ...
        if use_bias:
            ...
            fused_bias += bias_
        bias_var1 = tf.compat.v1.Variable(initial_value=fused_bias, name="%s_b" % name)
    ...
    def DepthWise_FuseBN_with_jsonParams(self, x, in_channels, channel_multiplier, kernel_size, stride,
        ...
        if use_bias:
            ...
            fused_bias += bias
        bias_var = tf.compat.v1.Variable(initial_value=fused_bias, name="%s_b" % name)
